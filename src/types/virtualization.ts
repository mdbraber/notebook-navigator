...
export interface ListPaneItem {
    ...
    // Property value node id for a single-value property group header. Lets the header inherit that
    // value's icon and colour from the navigation tree. Absent for multi-value joined groups, for the
    // no-value group, and for tag values, which carry headerTagPath instead.
    headerPropertyNodeId?: string | null;
    // Tag tree path for a group header whose value is a tag, so the header inherits the tag row's icon
    // and colour. Mutually exclusive with headerPropertyNodeId.
    headerTagPath?: string | null;
